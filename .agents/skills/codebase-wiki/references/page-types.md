...
| `synthesis` (Codebase audit) | Static functional-capability and entrypoint audit across current source, configuration, transactions, logic/state contracts, and targeted Git history | `assets/code-audit-template.md` | NotebookLM-excluded local report, stable `FUNC-*` capability rows, `BUG-*`/`RISK-*`/`BIZ-*` IDs, source-backed coverage, rerun states, historical evidence, gaps, and preserved user notes |
