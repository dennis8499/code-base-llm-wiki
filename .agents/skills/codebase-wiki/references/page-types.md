...
| `synthesis` (Codebase audit) | Static entrypoint audit across current source, configuration, transactions, logic/state contracts, and targeted Git history | `assets/code-audit-template.md` | NotebookLM-excluded local report, stable `BUG-*`/`RISK-*`/`BIZ-*` IDs, source-backed coverage, historical evidence, gaps, and preserved user notes |
