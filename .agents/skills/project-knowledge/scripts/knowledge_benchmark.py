...
SAMPLE_COUNT = 3
REPORT_SCHEMA = "knowledge-portability-report/v3"
OPERATION_ORDER = (
...
def _validated_controlled_operation_samples(
    controlled_operation_samples: dict[str, list[float]] | None,
) -> dict[str, list[float]] | None:
    """Validate the optional in-process timing seam before fixture mutation."""

    if controlled_operation_samples is None:
        return None
    if not isinstance(controlled_operation_samples, dict) or list(
        controlled_operation_samples
    ) != list(OPERATION_ORDER):
        raise ValueError("controlled samples do not match the closed operation order")
    normalized: dict[str, list[float]] = {}
    for operation_id in OPERATION_ORDER:
        samples = controlled_operation_samples[operation_id]
        if not isinstance(samples, list):
            raise ValueError("controlled samples must use lists")
        normalized[operation_id] = classify_operation(samples)["samples_seconds"]
    return normalized
...
def run_benchmark(
    ...
    *,
    ...
    controlled_operation_samples: dict[str, list[float]] | None = None,
) -> dict[str, Any]:
    """Build the fixture, then measure fresh cold and compatible warm paths."""

    controlled_samples = _validated_controlled_operation_samples(
        controlled_operation_samples
    )
    measurement_mode = "controlled" if controlled_samples is not None else "observed"
    ...
    try:
        ...
        for index, query in enumerate(QUERY_TOKENS):
            ...
            for sample_index in range(SAMPLE_COUNT):
                ...
                operation_samples[operation_id].append(
                    controlled_samples[operation_id][sample_index]
        ...
        for sample_index in range(SAMPLE_COUNT):
            ...
            operation_samples["index_candidate"].append(
                controlled_samples["index_candidate"][sample_index]
                if controlled_samples is not None
                else index_duration
            )
