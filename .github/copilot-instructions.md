...
- Codebase audit 以原生搜尋與直接讀取靜態追查入口，交叉檢查 transaction、設定引用、邏輯／
  狀態與定向 Git history；不使用 tgrep，也不執行目標程式、測試或修正。明確健檢請求授權
  保存報告；指定「只回報」則不寫 Wiki、index 或 log。finding 分為 `BUG-*`、`RISK-*`、`BIZ-*`，
  保存後執行 `validate-code-audit.py`。
