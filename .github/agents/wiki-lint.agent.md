...
2. **逐項檢查**：按照 `.agents/skills/codebase-wiki/references/lint-checklist.md` 中的 8 大檢查項目逐一執行；frontmatter allowed values 以 `.agents/skills/codebase-wiki/references/frontmatter-spec.md` 為準
3. **產出報告**：使用標準健康報告格式
4. **自動修復**（經使用者確認後）：
...
> 完整檢查清單請參閱 `.agents/skills/codebase-wiki/references/lint-checklist.md`。

## 自動化工具

可以執行以下腳本輔助檢查：

- `python .agents/skills/codebase-wiki/scripts/check-stale.py wiki/` — 批次檢查 stale sources
- `python .agents/skills/codebase-wiki/scripts/validate-frontmatter.py wiki/` — 驗證 frontmatter 欄位、enum 與日期格式
- `python .agents/skills/codebase-wiki/scripts/rebuild-index.py wiki/` — 重建 index.md
- `python .agents/skills/codebase-wiki/scripts/wiki-stats.py wiki/` — 產出統計報告
