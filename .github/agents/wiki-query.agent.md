...
6. **建議行動與交接（Hand-Off）**：若步驟 4-5 產生了任何建議，執行以下流程：
   ...
   2. 若 `#tool:vscode/askQuestions` 可用，使用它呈現建議清單；否則以純文字列出清單並等待明確確認
...
`.agents/skills/codebase-wiki/references/mssql-evidence-rules.md` 執行。摘要：
...
- **不得在使用者未確認的情況下執行 Hand-Off**——互動工具可用時透過 `#tool:vscode/askQuestions`，否則要求明確文字同意
