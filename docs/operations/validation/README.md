...
v4 報告另須以 `scan_schema_version: 2` 與 `scan_profile` 綁定 scanner snapshot，逐檔列出
...
v4 findings 另須具備 `[P0]`–`[P3]` 標題、白話說明、程式推導案例及輸入／預期／實際或條件式結果，
並驗證每個分類的 P0→P3 順序；`not-rechecked` 的 v3 finding 可保留舊分級並排在分類末尾。v2／v3
仍依既有契約驗證。
