...
16. 請以時間順序說明完整的日終關帳流程：觸發、前置檢核、每一步處理、資料讀寫、狀態變更與成功結果。
17. 若其中一個前置檢核或下游依賴失敗，確切的阻擋條件、失敗去向與已保存狀態是什麼？
18. 淨值或狀態寫入中途失敗時，哪些變更會保留、哪些會回復？後續通知如何處理？
19. 重跑或重送同一營業日的關帳會發生什麼事？若核准角色、重跑權限或 SLA 沒有來源，是否明確保留未知？
20. shared business context 是否包含流程步驟、例外分支與適用規則正文，而非只有標題或 Wiki 連結？
...
第 1–10 題計入 BA 20 分；第 11–15 題是 BA／SA 追溯必過 gates；第 16–20 題是流程深度與
來源完整性 gates，不宣稱 NotebookLM 生成式問答已由 exporter 驗證。通過條件：
...
- 第 16 題必須按順序保留觸發、條件、每一步行為、資料／狀態、結果與失敗分支；只有流程標題、四步摘要或規則連結不得給滿分；
- 第 17–19 題必須保留中途失敗、已保存狀態與重跑行為，並分開標示 `analysis-gap`、`evidence-gap`、`business-confirmation`；不得用 `Codebase 未提供證據` 掩蓋尚未追查完成；
- 第 20 題必須能在實際 `sources/*.md` 找到流程正文、分支與適用需求／規則正文；結構檢查通過仍要把 NotebookLM 問答列為未驗證，直到人工實測。
