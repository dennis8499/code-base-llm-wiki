...
- **2 分**：直接回答題意，引用 BA 主文件；規則附證據狀態，未知事項明確列為 gap；需要時才引用 business evidence 或 traceability。
...
- 每個事實都能追溯至 BA 文件或 business evidence，推論與 gap 不得偽裝成既定規則；
- 回答先使用 `business-process`、`business-rule`、glossary 與 gaps，技術頁只能作補充；
...
| 只能從 code/path 回答 | 補 BA 主文件；保留 code/path 於 traceability appendix |
