...
source_digest: sha256:31ea7df6beedfca75387b67894175501e12182cec13ae56f33abd7aca8295c97
...
| Non-functional requirements | partial | correctness/atomicity、200-page lint、500-module Wiki full preflight/apply 與 50k/5k 三樣本 query/index benchmark contract covered；實際 Linux portability 與 NotebookLM retrieval benchmark 仍是 gap |
...
| 效能 | 無常駐服務與第三方 runtime；`query-index` 為 bounded Markdown router；200-page lint、500 個 synthetic module 的 full preflight/apply，以及 50,000 files／5,000 pages 的五個 cold、五個 warm、單一 index 三樣本四態 benchmark contract | 尚無實際 Linux strict-clean portability comparison 與真實 NotebookLM retrieval benchmark |
...
- Benchmark functional drift 優先於 timing；mixed evidence 為 inconclusive、持續超限為
  performance failure，environment/cleanup failure 使用安全 v2 error 且不能成為 portability pass。
