...
```text
每次執行都先執行 `.agents/skills/codebase-wiki/scripts/scan-project.py --root . --profile target --format json`；先從目前 Codebase 的目錄、manifest、設定與入口註冊處盤點，重新建立 inventory，包含子目錄、巢狀 repository、ignored／untracked 來源及自有 CI/CD、IaC、scripts、tools、bin；再盤點 API、UI、CLI、排程、事件、plugin 與公開介面，沿每個入口追查目前 source 的輸入、驗證／權限、業務邏輯、資料／狀態、輸出與失敗路徑。掃描範圍不由 Wiki 決定，只有遇到業務規則或政策語意缺口才查 Wiki。交叉檢查 transaction、configuration、logic/state、change-completeness 四類，並在 source 追查後使用 git log、git show、git blame 核對定向歷史；遇到 merge commit 時逐一比較每個 parent 與 merge 結果，核對驗證、授權、錯誤處理、設定與資料轉換是否遺失，並以目前 source 確認仍可達。分開列出確定缺陷、技術風險與待確認政策，合併相同根因，記錄每個入口的 checked／partial／not checked 覆蓋、每個檔案的處置與 scan snapshot，保存繁中 v4 報告到 wiki/synthesis/code-audit-all.md。Finding 先用白話說明，再列 `操作／輸入`、`預期結果`、`實際結果` 的程式推導案例（未實際執行），依 P0→P3 排序；更新相關 Wiki link、index 與 synthesis log，最後執行 validate-code-audit.py。不要執行程式、測試或修正。
Chat summary: explain the main findings in plain language, include one short operation/input case, link the report, disclose checked/partial/not checked coverage limits, and name any incomplete scope without claiming the whole project is clean.

以穩定的 `FUNC-*` 功能／使用情境整理結果，再以入口逐項核對覆蓋；一個功能可以包含多個 API、UI、CLI、排程、事件或公開介面。每個功能都要記錄相關入口、已檢查的正常／邊界／錯誤／授權／狀態／交易／設定／相容性／效能情境與 checked／partial／not checked 狀態；無法歸類的入口使用 `FUNC-UNCLASSIFIED-{slug}`，不得遺漏。每個 finding 同時記錄受影響功能與入口，每個掃描檔案在 v4 `檔案處置` 表中逐檔列出。報告使用 `audit_report_version: 4`、`scan_schema_version: 2` 與 `scan_profile: target|framework`，validator 依 profile 重跑 shared scanner 並比對 snapshot、完整 path set、category 與 disposition；重跑狀態分為 `new`、`still-present`、`rechecked-no-longer-observed`、`not-rechecked`，沿用既有 finding IDs、user notes 與已移除入口的歷史；新 finding 在各分類中依 P0 → P1 → P2 → P3 排序，未重查的舊 finding 保留原文與原分級置於末尾。
```
...
through the shared capability and workflow contracts. Codebase audit likewise
keeps application source read-only while using targeted `git log`, `git show`,
and `git blame` as historical evidence; it records `BUG-*`, `RISK-*`, and
`BIZ-*` separately and validates persisted reports with
`validate-code-audit.py`. Prompt metadata does not grant writes, and shell
access is not a technical write sandbox; host permissions must deny unapproved
shell writes. Hook guards remain a defense-in-depth layer and do not replace
platform sandbox or host permission controls.
...
# 若已建立持久化 Codebase audit report，再執行下一行
python .agents\skills\codebase-wiki\scripts\validate-code-audit.py wiki\synthesis\code-audit-all.md --repo-root .
